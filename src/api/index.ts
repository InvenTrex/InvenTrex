...
const registerRoutes = (app: Express, dir: string, method: 'post' | 'get' | 'delete') => {
  ...
  files.forEach(file => {
    ...
    import(file).then(module => {
      ...
      if (typeof handler === 'function') {
        app[method](routePath, async (req: Request, res: Response, next: NextFunction) => {
          try {
            await handler(req, res);
          } catch (err) {
            Logger.severe('express', `Error handling ${method.toUpperCase()} ${routePath}: ${err.message}`);
            res.status(500).json({ error: 'Internal Server Error' });
          }
        });
      } else {
        Logger.warning('express', `No 'execute' function found in ${file}`);
      }
    }).catch(err => {
      Logger.severe('express', `Failed to load module ${file}: ${err.message}`);
    });
  });
};
